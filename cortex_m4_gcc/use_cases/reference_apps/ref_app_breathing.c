...
#include "ref_app_breathing_main.h"


#define B_STATIC_LENGTH (3U)//3u
#define A_STATIC_LENGTH (2U)//2u
#define B_ANGLE_LENGTH  (5U)//5u
#define A_ANGLE_LENGTH  (4U)//4u
...
ref_app_breathing_config_t *ref_app_breathing_config_create(void)
{
	...
	if (config != NULL)
	{
		...
		acc_detector_presence_config_frame_rate_set(presence_config, 5.0f);
...
ref_app_breathing_handle_t *ref_app_breathing_create(ref_app_breathing_config_t *config)
{
	...
	ref_app_breathing_handle_t *handle = acc_integration_mem_calloc(1, sizeof(*handle));
