 ...
 */
static void obstruction_signature(acc_int16_complex_t *subsweep, float *magnitudes, float *distances, uint16_t length, float noise_level,
...
bool ref_app_parking_obstruction_calibration(ref_app_parking_handle_t *handle)
{
	...
	for (uint16_t i = 0; i < CAL_ITERATIONS; i++)
	{
		...
		obstruction_signature(subsweep, handle->obstruction_magnitudes, handle->obstruction_distances, obs_cal_data_length, noise_levels[i],
		                      &signature);
...
void ref_app_parking_obstruction_process(ref_app_parking_handle_t *handle, bool *obstruction_detected)
{
	...
	acc_processing_get_temperature_adjustment_factors(handle->noise_cal_temp, handle->proc_result.temperature, profile, &signal_adjust_factor,
	...
	// Extract signature
	obstruction_signature(subsweep, handle->obstruction_magnitudes, handle->obstruction_distances, num_points, noise_level_adjusted,
	                      &curr_signature);
	...
	*obstruction_detected = (energy_diff > handle->obstruction_energy_threshold) ||
	                        (weighted_distance_diff > handle->obstruction_weighted_distance_threshold);
}
...
static void set_sensor_config(ref_app_parking_handle_t *handle)
{
	...
	uint16_t num_points = (uint16_t)ceilf(
		(car_config->range_end_m - car_config->range_start_m) / (step_length * ACC_APPROX_BASE_STEP_LENGTH_M));
	...
	if (parking_config->obstruction_detection_enabled)
	{
		...
		num_points = (uint16_t)ceilf(
			(obstruction_config->range_end_m - obstruction_config->range_start_m) / (step_length * ACC_APPROX_BASE_STEP_LENGTH_M));
...
static float noise_process(acc_int16_complex_t *noise_data, float *noise_magnitudes, uint16_t noise_data_length)
{
	...
	{
		noise_magnitudes[i] = sqrtf(
			(float)noise_data[i].real * (float)noise_data[i].real + (float)noise_data[i].imag * (float)noise_data[i].imag);
...
static float obstruction_noise(acc_int16_complex_t *obs_noise_data, uint16_t obs_noise_data_length)
{
	...
	{
		noise_mean += sqrtf(
			(float)obs_noise_data[i].real * (float)obs_noise_data[i].real +
			(float)obs_noise_data[i].imag * (float)obs_noise_data[i].imag);
	}
