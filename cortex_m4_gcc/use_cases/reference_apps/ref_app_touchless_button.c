...
#define SENSOR_TIMEOUT_MS (1000U)//1000U 
...
#define DEFAULT_PRESET ACC_TOUCHLESS_BUTTON_PRESET_FAR
...
int acc_ref_app_touchless_button(int argc, char *argv[]);
...
static void set_config(acc_touchless_button_config_t *config, acc_touchless_button_preset_t preset)
{
	switch (preset)
	{
		...
		case ACC_TOUCHLESS_BUTTON_PRESET_CLOSE:
			acc_config_sweeps_per_frame_set(config->sensor_config, 16U);//16U
			acc_config_sweep_rate_set(config->sensor_config, 320U);//320U
			...
			acc_config_num_points_set(config->sensor_config, 3U);//3U
			acc_config_profile_set(config->sensor_config, ACC_CONFIG_PROFILE_1);
			acc_config_receiver_gain_set(config->sensor_config, 0U);//0U
			acc_config_hwaas_set(config->sensor_config, 40U);//40U
			acc_config_start_point_set(config->sensor_config, 0U);//0U
			acc_config_step_length_set(config->sensor_config, 6U);//6U

			config->sensitivity_close      = 1.9f;//1.9f
			config->patience_close         = 2U;//2U
			config->calibration_duration_s = 0.6f;//0.6f
			config->calibration_interval_s = 20.0f;//20.0f
			config->measurement_type       = ACC_TOUCHLESS_BUTTON_CLOSE_RANGE;
			break;
		case ACC_TOUCHLESS_BUTTON_PRESET_FAR:
			acc_config_sweeps_per_frame_set(config->sensor_config, 16U);//16U
			acc_config_sweep_rate_set(config->sensor_config, 200U);//320U
			acc_config_continuous_sweep_mode_set(config->sensor_config, true);
			acc_config_double_buffering_set(config->sensor_config, true);
			acc_config_inter_sweep_idle_state_set(config->sensor_config, ACC_CONFIG_IDLE_STATE_READY);
			acc_config_inter_frame_idle_state_set(config->sensor_config, ACC_CONFIG_IDLE_STATE_READY);
			acc_config_num_points_set(config->sensor_config, 3U);//3U
			acc_config_profile_set(config->sensor_config, ACC_CONFIG_PROFILE_1);
			acc_config_receiver_gain_set(config->sensor_config, 5U);//5U
			acc_config_hwaas_set(config->sensor_config, 60U);//60U
			acc_config_start_point_set(config->sensor_config, 0U);//0U
			acc_config_step_length_set(config->sensor_config, 24U);//24U

			config->sensitivity_far        = 2.0f;//2.0f
			config->patience_far           = 2U;//2U
			config->calibration_duration_s = 0.6f;//0.6f
			config->calibration_interval_s = 20.0f;//20.0f
			config->measurement_type       = ACC_TOUCHLESS_BUTTON_FAR_RANGE;
			break;
		case ACC_TOUCHLESS_BUTTON_PRESET_CLOSE_AND_FAR:
			acc_config_num_subsweeps_set(config->sensor_config, 2U);//2U
			acc_config_sweeps_per_frame_set(config->sensor_config, 16U);//16U
			acc_config_sweep_rate_set(config->sensor_config, 320U);//320U
			acc_config_continuous_sweep_mode_set(config->sensor_config, true);
			acc_config_double_buffering_set(config->sensor_config, true);
			...
			acc_config_subsweep_num_points_set(config->sensor_config, 3U, 0U);//3U,0U
			acc_config_subsweep_profile_set(config->sensor_config, ACC_CONFIG_PROFILE_1, 0U);//0U
			acc_config_subsweep_receiver_gain_set(config->sensor_config, 0U, 0U);//0U,0U
			acc_config_subsweep_hwaas_set(config->sensor_config, 40U, 0U);//40U,0U
			acc_config_subsweep_start_point_set(config->sensor_config, 0U, 0U);//0U,0U
			acc_config_subsweep_step_length_set(config->sensor_config, 6U, 0U);//6U,0U

			acc_config_subsweep_num_points_set(config->sensor_config, 3U, 1U);//3U,1U
			acc_config_subsweep_profile_set(config->sensor_config, ACC_CONFIG_PROFILE_3, 1U);//1U
			acc_config_subsweep_receiver_gain_set(config->sensor_config, 5U, 1U);//5U,1U
			acc_config_subsweep_hwaas_set(config->sensor_config, 60U, 1U);//60U,1U
			acc_config_subsweep_start_point_set(config->sensor_config, 0U, 1U);//0U,1U
			acc_config_subsweep_step_length_set(config->sensor_config, 24U, 1U);//24U,1U
